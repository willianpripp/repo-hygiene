...
octocat/hello-world [public-portfolio/github] public
  FAIL  license_file                 no LICENSE* file at root
        > Add a LICENSE file at the repo root. Pick one at https://choosealicense.com/ if unsure.
  warn  readme_has_structure         no heading matches /(project structure|...)/i
        > Add a "## Project structure" heading with a fenced tree of the top two
          levels. Generate it from the actual layout rather than from memory.
  skip  env_example_present          no compose/Dockerfile/.env.example marker
  ok    14 passed: has_description, has_topics, readme_links_resolve, ...

SUMMARY
repo                                        MUST fail  SHOULD fail
octocat/hello-world                                 1            1
```

Exit `0` when every MUST passes, `1` when one does not, `2` when the run
itself could not complete. That last one matters: a check that silently fails
to run is worse than no check, so an unfinished run never reports clean.
...
Two checks walk relative markdown links and confirm each target exists:
`readme_links_resolve` for the README, `docs_links_resolve` for every markdown
file under `docs/`. Links resolve relative to the file that contains them, so a
`../../README.md` inside `docs/history/` is checked the way a reader's browser
would follow it.

These are the checks that catch documentation rot, and the second one exists
because of a concrete failure. Splitting a long status file into `docs/history/`
moved its content two directories down, and every link it carried had been
written relative to the repo root. Eighteen links broke silently in a commit
whose entire purpose was tidying the documentation. The README check saw none
of it, because none of them were in the README.

## Project structure

```
repo-hygiene/
├── checklist.yaml        # the standard: profiles, tiers, thresholds
├── scripts/audit.py      # the only executable: reads the checklist, reports
├── action.yml            # the composite action, so consuming this is one step
├── WORKFLOW_TEMPLATE.yml # drop-in workflow for a repo that wants checking
├── Makefile              # make check, make self-test
└── .github/workflows/    # this repo checking itself, with the bar in the commit under test
```

Two files carry everything. `checklist.yaml` says what the bar is;
`scripts/audit.py` executes it and knows nothing about which repos exist. The
private half (which repo gets which profile) lives in an overlay file that
never ships here.
